...
def export_opt_data(results,model,tList,patTypeList,hosIdList,areaIdList,qtdPuraCovidReal,
                    ...
                    InitPatientsph,releasePatientspht,LOSp,Wdist,Winf,qtdTT):
    debugList = []
    graphList = []
    acuAlocacao = 0
    ...
    if 'ok' == str(results.Solver.status):
        ...
        df.to_csv(f'./data/processed/debug_D{Wdist}_I{Winf}.csv',index=False)
        graph = pd.DataFrame(graphList,columns=['Timestamps','Source','Target','Weight'])
        graph.to_csv(f'./data/processed/edgeGraph/edgeGraphSimullated_D{Wdist}_I{Winf}.csv',index=False)
    else:
